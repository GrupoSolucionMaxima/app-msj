...
const mapArticleForEventDos = (a: ArticleFromApi): ItemDos => {
  let imageUrl = '';
...
export default function HomeScreen() {
  ...
  const [dataExpriences, setDataExperiences] = useState<ItemDos[]>([]);
    ...
    const fetchExperencia = async () => {
      ...
      try {
        ...
        const filteredDataInfo = mappedArticles
          ...
          .map(mapArticleForHome);
        // 4. FILTRAR POR "Info al viajero"
        const filteredDataEvents = mappedArticles
          .filter(item =>
            item.category.toLowerCase().includes("eventos")
          )
          .map(mapArticleForEventDos);
        setData(filteredData);
        setDataInfo(filteredDataInfo);
        setDataExperiences(filteredDataEvents);
      } catch (e: any) {
