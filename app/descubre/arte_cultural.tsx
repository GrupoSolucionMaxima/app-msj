...
const mapArticleForHome = (a: ArticleFromApi): HomeContentItem => {
  ...
  const categoryColor = a.category.toLowerCase().includes("arte y cultura") ? 'bg-[#EE3048]' : 'bg-primary';
...
export default function ArteCultural() {
    ...
    const fetchArteCultura = async () => {
      ...
      try {
        ...
        const filteredData = mappedArticles
          .filter(item =>
            item.category.toLowerCase().includes("arte y cultura")
          )
  ...
  const renderContent = () => {
    ...
    if (data.length === 0) {
      return (
        <View style={styles.statusContainer}>
          <Text style={styles.statusText}>No se encontró contenido de Arte y cultura.</Text>
        </View>
      );
    }
