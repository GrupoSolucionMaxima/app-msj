...
const mapArticleForHome = (a: ArticleFromApi): HomeContentItem => {
  ...
  const categoryColor = a.category.toLowerCase().includes("deporte y recreación") ? 'bg-[#EE3048]' : 'bg-primary';
...
export default function DepertoRecreacion() {
  ...
  useEffect(() => {
    const fetchDeporteCultura = async () => {
      setLoading(true);
      setError('');
      try {
        ...
        // 3. FILTRAR POR "Arte y cultura"
        const filteredData = mappedArticles
          .filter(item =>
            item.category.toLowerCase().includes("deporte y recreación")
          )
        ...
        console.error("Fetch Error en Arte y cultura: ", e);
  ...
  const renderContent = () => {
    ...
    if (data.length === 0) {
      return (
        <View style={styles.statusContainer}>
          <Text style={styles.statusText}>No se encontró contenido de Deporte y recreación.</Text>
        </View>
      );
    }
