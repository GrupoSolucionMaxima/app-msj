...
const mapArticleForHome = (a: ArticleFromApi): HomeContentItem => {
  ...
  const categoryColor = a.category.toLowerCase().includes("patrimonio") ? 'bg-[#EE3048]' : 'bg-primary';
...
export default function patrimonio() {
  ...
  useEffect(() => {
    const fetchArmoniaUrbana = async () => {
      setLoading(true);
      setError('');
      try {
        ...
        // 3. FILTRAR POR "patrimonio"
        const filteredData = mappedArticles
          .filter(item =>
            item.category.toLowerCase().includes("patrimonio")
          )
        ...
        console.error("Fetch Error en patrimonio:", e);
  ...
  const renderContent = () => {
    ...
    if (data.length === 0) {
      return (
        <View style={styles.statusContainer}>
          <Text style={styles.statusText}>No se encontró contenido de patrimonio.</Text>
        </View>
      );
    }
